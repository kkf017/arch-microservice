...
def run()->None:
	...
	if not microservice.core.handling_datetime.handling_datetime.check_datetime_files():
		...
		return
	try:
		columns = microservice.infrastructure.sql_driver.get_columns_name(DATABASE, TABLE)
		x = microservice.infrastructure.sql_driver.request(DATABASE, f'''SELECT * FROM {TABLE};''')
